      ...
      <GridContainer style={{ height: '100%', flexDirection: 'column' }}>
        ...
        <GridContainer style={{ height: 'calc(100vh - 300px)' }}>
          <GridItem sm={12} md={4} lg={4}>
            <Paper style={{ height: '100%', minWidth: '250px' }} className={classes.paper}>
              <Typography
                variant="div"
                gutterBottom
                style={{
                ...
                {!chatUser && !newChatUser ? (
                  ...
                  <span style={{ display: 'flex', alignItems: 'center' }}>
                    <img
                      onClick={() =>
                        Router.push(
                          `/profile?slug=chats&user=${chatUser ? chatUser.id : newChatUser.id}`,
                          `/profile/chat/user/${chatUser ? chatUser.id : newChatUser.id}`,
                          { shallow: true }
                        )
                      }
                      src={
